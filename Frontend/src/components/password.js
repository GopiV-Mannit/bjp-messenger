        ...
        <form onSubmit={handleSubmit}>
          <div className="flex flex-col mt-5  h-[475px] w-[350px] items-center  mr-36 bg-[#00a650] rounded-2xl shadow-5xl border-r-0 border-b-0 border-opacity-30">
            <div className="flex flex-col items-center justify-center m-6 ">
              <div className=" p-1 rounded-full m-10">
                <div className="rounded-full m-1">
                  <img className="h-15 w-20 rounded-full shadow-[0px_0px_15px] shadow-white bg-blue-500" src={logo} alt="user" />
                </div>
              </div>
              <div>
                <p className="text-xl text-[#fff]"> Change Password</p>
              </div>
            </div>
            <div className="flex flex-col space-y-[24px] mt-2">
              <div className=' flex flex-row space-x-3'>
                <FontAwesomeIcon icon={faUser} className='text-[#fff] mt-1' />
                <input value={user} onChange={e => userchange(e.target.value)} type="text" placeholder="Enter username" className="text-[15px] w-52  text-sm   text-[#fff] placeholder-[#fff] bg-transparent focus:outline-none border-b-2  border-[#fff] required"
                  required
                  pattern=".*\S+.*"
                />
              </div>
              <div className="flex flex-row space-x-3">
                ...
                <div className="flex flex-row">
                  <input
                    ...
                    className="w-52 text-sm text-[#fff] placeholder-[#fff] bg-transparent focus:outline-none border  border-[#fff] border-r-0 border-t-0 border-l-0 border-b-2 pr-10"
                  />
                  <button
                    type="button"
                    className=" absolute top-1/6  transform -translate-y-[4px] ml-48 cursor-pointer bg-[#00a650] focus:outline-none border-b-[0.1px] border-[#fff]"
            ...
            <div className="flex flex-col justify-center mt-10">
              <button className="text-white font-bold  h-8 w-40  ml-4 text-sm cursor-pointer font-poppins rounded-sm bg-[#fb660d] border " >
                Change
              </button>
              <div className='flex flex-row  mt-5  space-x-[30px] justify-center'>
                <p className='text-[#fff] text-sm'>Go to Login ?</p>
